      ...
      <div className="overflow-hidden touch-pan-y" onTouchStart={onTouchStart} onTouchEnd={onTouchEnd}>
        <div
          ...
          style={{
            ...
            transition: transition ? "transform 400ms cubic-bezier(0.16, 1, 0.3, 1)" : "none",
          }}
          ...
          {extended.map((item, idx) => {
            ...
            return (
              <div key={idx} className="w-[85%] flex-shrink-0 px-2.5">
                <div
                  className={cn(
                    "h-full transition-all duration-500 ease-out origin-center",
                    active ? "scale-100 opacity-100" : "scale-[0.97] opacity-55",
                  )}
                >
                  {renderItem(item, idx)}
                </div>
